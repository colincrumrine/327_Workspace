...
{	 
	...
	}else if(l == 0 && s == 1 && nummon == 1)
	{
		...
		simulate(nummonCount,hardness,roomStats,pcLocation, carry, equipment, 200000);
...
{	
	 ...
	 for(k = 0; k < 24; k++)
	 {
		savedRooms[k] = roomStats[k];
			...
			FILE *fp;
			char *path;
			path = (char *) malloc(strlen(getenv("HOME")) + strlen("/.rlg327/dungeon") + 1);
			strcpy(path, getenv("HOME"));
			strcat(path,"/.rlg327/dungeon");
			fp = fopen(path, "w");
			free(path);			
			
			char fileMarker[13] = "RLG327-F2018";
	...
	for(j = 0; j < 12; j++)
	{
		equipment[j] = NULL;
			...
			if(summon == 1)
			{
				if(monsterCount > 0)
				{
					simulate(monsterCount,readHardness,simulationRooms,readPCLoc, carry, equipment, 200000);
				}else{
					simulate(0,readHardness,simulationRooms,readPCLoc, carry, equipment, 200000);
				}
			}	
...
object* carry[10];
	...
	for(j = 0; j < 12; j++)
	{
		equipment[j] = NULL;
			...
			FILE *f;
			char *p;
			p = (char *) malloc(strlen(getenv("HOME")) + strlen("/.rlg327/dungeon") + 1);
